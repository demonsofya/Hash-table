...
void DrawDotNodes(list_t *list_struct, FILE *dot_file_ptr) {
    ...
    snprintf(curr_node_args->label, MAX_LABEL_STRING_SIZE, "data = %s | index = %d"
                                                          "| { HEAD = %d | TAIL = %d }",
    ...
    for (int i = 1; i < list_struct->list_size; i++) {
         ...
         snprintf(curr_node_args->label, MAX_LABEL_STRING_SIZE,
                                        "data = %s | index =  %d | { next = %d | prev = %d }",
                                        list_struct->data[i], i, list_struct->next[i],
                                        list_struct->prev[i]);

         if (i == list_struct->free) {
            snprintf(curr_node_args->label, MAX_LABEL_STRING_SIZE,
                                           "FREE | data = %s | index = %d | { next = %d | prev = %d }",
                                           list_struct->data[i], i, list_struct->next[i],
                                           list_struct->prev[i]);
            ...
            snprintf(curr_node_args->label, MAX_LABEL_STRING_SIZE,
                                           "data = %s [POISON] | index =  %d | { next = %d | prev = %d }",
                                           list_struct->data[i], i, list_struct->next[i],
                                           list_struct->prev[i]);
